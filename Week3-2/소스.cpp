...
int main()
{
	...
	inputFile2 = fopen("AICenter_Noise.bmp", "rb");
	...
	for (int j = 0; j < height; j++)
	{
		for (int i = 0; i < width; i++)
		{
			...
			Y1 = (Y1 > 255) ? 255 : (Y1 > 0 ? Y1 : 0);
			Y2 = inputImg2[j * stride + 3 * i + 0];
	...
	printf("MSE = %.2lf\nPSNR = %.2lf dB\n", mse, psnr);
	FILE* outputFile = fopen("AICenter_ERR55.bmp", "wb");//Output: copy pictures, Output_RED: red pictures, Output_Blue: blue pictures, Output_GREEN: green pictures
